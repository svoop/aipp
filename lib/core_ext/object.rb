class Object
  ...
  # @param severe [Boolean] whether this problem should be fixed
  # @param pry [Exception, Binding, nil] attach the Pry session to this error
  #   or binding
  def warn(message, severe: true, pry: nil)
    if severe || $UNSEVERE_WARN
      $WARN_COUNTER = $WARN_COUNTER.to_i + 1
      Kernel.warn "WARNING #{$WARN_COUNTER}: #{message} #{'(unsevere)' unless severe}".red
